...
def GCNtrain(model, traindataloader, criterion, traindataset, optimizer, device, num_epochs, labels, valdataset, valdataloader, num_workers, save_dir, testdataloader, testdataset):
    ...
    for epoch in range(num_epochs):
        ...
        train_accuracy, train_map, train_auc = evaluate_multilabel_classification(targets_np, predictions_np)
        ...
        with torch.no_grad():
            ...
            val_accuracy, val_map, val_auc = evaluate_multilabel_classification(targets_np, predictions_np)
    ...
    with torch.no_grad():
        ...
        epoch_outputs = torch.empty((0,len(labels)))
        epoch_targets = torch.empty((0,len(labels)))
        for inputs, targets, edges, image in testdataloader:
            mean = inputs.mean(dim=2, keepdim=True)[0]
            std = inputs.std(dim=2, keepdim=True)[0]
            inputs = (inputs - mean) / (std + 1e-8) 
            # print(inputs)
        ...
        targets_np = epoch_targets.numpy()
        print("Test metrics:\n")
        test_accuracy, test_map, test_auc = evaluate_multilabel_classification(targets_np, predictions_np)
