...
def GCNtesthierarchy(model, traindataloader, criterion, traindataset, optimizer, device, num_epochs, labels, valdataset, valdataloader, num_workers,  testdataloader, testdataset, trainfreq, valfreq,testfreq):
    ...
    epoch_outputs = torch.empty((0,len(labels)))
    epoch_targets = torch.empty((0,len(labels)))
    for inputs, targets, edges, edges2, edges3, edges4, edges5,image, connectivity in testdataloader:
        ...
        batchx.x.retain_grad()
        optimizer.zero_grad()
        outputs = model(batchx, batchx2, batchx3, batchx4, batchx5)
        target_class = 2
        target_score = outputs[0,target_class]
        # print(target_score)
        ...
        # print(batchx.x)
        saliency = batchx.x.grad.data.abs()
        torch.save(saliency, 'saliencyGTHierarchytst35936.pth')
        # print(outputs)
        loss = criterion(outputs, targets)
        epoch_outputs = torch.cat((epoch_outputs, outputs.cpu()))
        epoch_targets = torch.cat((epoch_targets, targets.cpu()))
...
def inference():
    ...
    if config["model"] == 'GraphTransformerHierarchy':
        ...
        # criterion = nn.BCEWithLogitsLoss()  
        optimizer = optim.Adam(model.parameters(), lr=config["init_lr"])
        model, optimizer, start_epoch = load_checkpoint(model, optimizer, f"/scratch/RadChest/checkpoints/model_GraphTransformerHierarchy_lr_2e-05_bs_64_epochs_100_labels_17connectivity=allk=5_27-12-2024_23-46-48/model_14.pth", device)
        ...
        criterion = nn.BCEWithLogitsLoss(pos_weight= weights.to(device))  
        optimizer = optim.Adam(model.parameters(), lr=config["init_lr"])
        model, optimizer, start_epoch = load_checkpoint(model, optimizer, f"/scratch/RadChest/checkpoints/model_GraphTransformer_lr_2e-05_bs_64_epochs_100_labels_17connectivity=allk=5_28-12-2024_10-31-51/model_28.pth", device)
        # criterion = nn.BCEWithLogitsLoss()  
        ...
        if(config["model"] == "CTNet"):
            ...
            optimizer = optim.Adam(model.parameters(), lr=config["init_lr"])
            train_dataset = DatasetCTNet(annotations_file=config["dataset"]["train_labels"], image_dir=config["dataset"]["train_images"],labels = labels, exclude=read_file(config["exclude"]), split="train", data_augment=False)
            train_loader = DataLoader(train_dataset, batch_size=config["batch_size"], shuffle=True, num_workers=config["num_workers"])

            val_dataset = DatasetCTNet(annotations_file=config["dataset"]["val_labels"], image_dir=config["dataset"]["val_images"] ,labels = labels, exclude=read_file(config["exclude"]), split="val", data_augment=False)
            val_loader = DataLoader(val_dataset, batch_size=config["batch_size"], shuffle=False, num_workers=config["num_workers"])

            test_dataset = DatasetCTNet(annotations_file=config["dataset"]["test_labels"], image_dir=config["dataset"]["test_images"] ,labels = labels, exclude=read_file(config["exclude"]), split="test", data_augment=False)
            test_loader = DataLoader(test_dataset, batch_size=config["batch_size"], shuffle=False, num_workers=config["num_workers"])
